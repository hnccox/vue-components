...
let days = computed(() => getDaysInWeek(props.visibleDate))
let hours = computed(() => {
	var arr = []
...
function getDaysInWeek(date: Date): any {
	...
	const frontPadding = (date: Date) => {
		if (date.getDay() !== date.getFirstDayOfWeek()) {
			...
			while (dayOfWeek !== date.getFirstDayOfWeek()) {
				...
				days.unshift({ "index": copiedDate.getDay(), "datetime": copiedDate.valueOf(), "date": copiedDate.getDate(), "events": [] })
			}
	...
	frontPadding(date)
	days.push({ "index": date.getDay(), "datetime": date.valueOf(), "date": date.getDate(), "events": [] });
	backPadding(date)
