import OpenLayers from '/components/application-ui/data-display/maps/openlayers.vue';
...
const Template = (args) => ({
	...
	components: { Card, OpenLayers },
	// The story's `args` need to be mapped into the template through the `setup()` method
	setup() {
		return { args, ...Card.args };
	},
	// And then the `args` are bound to your component with `v-bind="args"`
	template: '<Card><template v-slot:body fullscreen=true><div class="grid grid-cols-1"><OpenLayers v-bind="args" /></div></template></Card>',
});

// const Template = (args) => <Mapbox v-bind="args"></Mapbox>

export const OpenlayersMap = Template.bind({});
